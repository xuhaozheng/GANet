...
class scared_dataset(data.Dataset):
    def __init__(self,
                 data_path,
                 filenames,
                 height,
    ...
    def get_image_path(self, folder, frame_index, side):
            ...
            image_path = os.path.join(self.data_path, folder, 'image_' + side_id, f_str)
            return image_path
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            if i == "s":
                ...
                inputs[("color", i, -1)] = self.get_color(folder, int(frame_idx), other_side)
            else:
                if self.acquire_gt == True:
                    inputs[("gt_depth")] = self.get_depth(folder, int(frame_idx) + i, side)
                inputs[("color", i, -1)] = self.get_color(folder, int(frame_idx) + i, side)
